...
class HPV_Bike_Ride_3D:
    '''
    Генерация ключей для игры Bike Ride 3D в Hamster Kombat
    -------------------------------------------------------
    ...
    def __init__(self, Console_Lock: Lock, _Thread: int, Proxy: dict = None) -> None:
        self.APP_TOKEN = 'd28721be-fd2d-4b45-869e-9f253b554e50'
...
def _HPV_Bike_Ride_3D(Console_Lock, Retry, Proxy) -> dict:
    '''Генерация ключей для игры Bike Ride 3D'''

    HPV_Keys = [] # Список хранящий ключи для Bike Ride 3D
    Threads = [] # Список потоков

    def HPV_Key_Gen(_Thread, Proxy=None):
        HPV = HPV_Bike_Ride_3D(Console_Lock, f'{_Thread} Bike', Proxy)
        KEY = HPV.Run()
    ...
    return {'Bike Ride 3D': HPV_Keys}, True if HPV_Keys else False
